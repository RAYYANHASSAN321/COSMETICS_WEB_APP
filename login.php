...
if(isset($_POST['signin'])){
    ...
    $query= "SELECT * FROM customer WHERE c_email = '$email' && c_password = '$password'";

    $runn =  mysqli_query($conn , $query);
  if(mysqli_num_rows($runn) > 0){
    $data = mysqli_fetch_array($runn);
    $_SESSION['coustmerLogin'] = $data['c_id'];
...
<html lang="en">
  ...
  <div class="logo text-center mt-3 mb-3">
    ...
    <input type="checkbox" onclick="myFunction()">&nbsp; Show Password
        ...
        <!-- Register Link -->
        <p class="text-center register-test mt-3">Don't have an account? <a href="resgister.php" class="text-decoration-none">Register here</a></p>
      </div>
  </div>
  <script>
    function myFunction() {
